...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setError('');
    setLoading(true);
    const { error, emailConfirmationRequired } = await signUp(email, password, fullName);
    setLoading(false);
    if (error) { setError(error); return; }
    if (emailConfirmationRequired) { setEmailConfirmationSent(true); return; }
  };
  ...
  if (emailConfirmationSent) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #F0F6FF 0%, #F8F9FA 60%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-inter), Inter, -apple-system, sans-serif' }}>
        <div style={{ width: '100%', maxWidth: 440, background: '#fff', borderRadius: 16, border: '1px solid #E3E8EF', boxShadow: '0 4px 24px rgba(0,0,0,.06)', padding: '48px 40px', textAlign: 'center' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>📬</div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#202124', marginBottom: 8 }}>Check your email</h1>
          <p style={{ fontSize: 14, color: '#5F6368', lineHeight: 1.7, marginBottom: 24 }}>
            We sent a confirmation link to <strong>{email}</strong>.<br />
            Click the link to activate your account and get started.
          </p>
          <button onClick={() => router.push('/login')} style={{ background: 'none', border: 'none', fontSize: 13.5, color: '#1A73E8', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            Back to sign in
          </button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #F0F6FF 0%, #F8F9FA 60%)', display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-inter), Inter, -apple-system, sans-serif' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 24px 60px' }}>
          <div style={{ width: '100%', maxWidth: 440, background: '#fff', borderRadius: 16, border: '1px solid #E3E8EF', boxShadow: '0 4px 24px rgba(0,0,0,.06)', padding: '36px 40px' }}>
            ...
            <button onClick={handleGoogleRegister} disabled={googleLoading} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '11px 16px', borderRadius: 9, border: '1.5px solid #E3E8EF', background: '#fff', fontSize: 14, fontWeight: 600, color: '#202124', transition: 'border-color .15s, box-shadow .15s', marginBottom: 20, cursor: googleLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
              onMouseEnter={e => { if (!googleLoading) { e.currentTarget.style.borderColor = '#1A73E8'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(26,115,232,.1)'; } }}
            ...
            <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Jane Smith" required
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1.5px solid #E3E8EF', fontSize: 14, color: '#202124', background: '#fff', transition: 'border-color .15s', boxSizing: 'border-box', fontFamily: 'inherit' }}
                  ...
                  <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {passwordChecks.map((c, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12 }}>
                        {c.ok ? <CheckIcon /> : <div style={{ width: 14, height: 14, borderRadius: '50%', border: '1px solid #D1D5DB', background: '#F9FAFB' }} />}
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '11px', borderRadius: 9, border: 'none', background: loading ? '#93C5FD' : '#1A73E8', color: '#fff', fontSize: 14, fontWeight: 700, transition: 'background .15s', marginTop: 4, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
                onMouseEnter={e => { if (!loading) e.currentTarget.style.background = '#1557B0'; }} onMouseLeave={e => { if (!loading) e.currentTarget.style.background = '#1A73E8'; }}>
