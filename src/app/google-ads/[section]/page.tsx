...
function filterCampaignsByNetworkMock(data: any[] | undefined, sectionText: string) {
  if (!data) return [];
  // Since we don't have the campaign type in DB, we'll pseudo route them to simulate functionality
  // for the specific pages by checking if campaign name includes the type (just a superficial mock)
  // or return all if it's empty so the UI doesn't look entirely broken.
  const lowerSec = sectionText.toLowerCase();

  let keyword = lowerSec;
  if (lowerSec === 'performance max') keyword = 'pmax';
  if (lowerSec === 'demand gen') keyword = 'demand';

  const matches = data.filter(c => c.name.toLowerCase().includes(keyword));
  // If no matches, return all for now to keep dashboard lively 
  // until we actually sync `campaign_type` in the database.
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <GoogleIcon size={24} />
            <h1 className="text-[22px] font-bold tracking-[-0.3px] text-[#202124]">
              Google Ads — {title}
            </h1>
          </div>
          <div className="mt-1">
...
export default function GoogleAdsSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  const filteredCampaigns = filterCampaignsByNetworkMock(campaignsQ.data, label);
