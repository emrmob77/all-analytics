...
/**
 * Validate that a URL uses http(s) — prevents javascript: / data: injection
 * in href attributes.
 */
...
function buildEmailHtml(payload: InvitationEmailPayload): string {
  const safeOrgName = escapeHtml(payload.orgName);
  const safeInviterName = escapeHtml(payload.inviterName);
  const roleLabel = escapeHtml(ROLE_LABELS[payload.role] ?? payload.role);
  // inviteUrl is validated in the handler before buildEmailHtml is called.
  const safeInviteUrl = payload.inviteUrl;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
          ...
          <tr>
            <td style="padding:36px 40px 32px;">
              ...
              <p style="margin:0 0 24px;font-size:14px;color:#5F6368;line-height:1.6;">
                ...
                <strong>${safeOrgName}</strong> in AdsPulse as a <strong>${roleLabel}</strong>.
              </p>
              ...
              <p style="margin:12px 0 0;font-size:12px;color:#9AA0A6;">
                ...
                <a href="${safeInviteUrl}" style="color:#1A73E8;word-break:break-all;">${escapeHtml(safeInviteUrl)}</a>
              </p>
            </td>
          </tr>
          <!-- Footer -->
...
Deno.serve(async (req: Request) => {
  ...
  if (!isSafeUrl(payload.inviteUrl)) {
    return new Response(
      JSON.stringify({ error: 'inviteUrl must use http or https protocol' }),
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    );
  }
